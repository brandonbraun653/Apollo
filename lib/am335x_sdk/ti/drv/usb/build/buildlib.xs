...
function createMake(makefile)
{
    ...
    if(makefile==undefined)
    {
      ...
      makefile.writeLine("\n# lib rule");                
      makefile.writeLine("lib: .executables");           
...
function createLibMake(device, objExtDir, makelibname,targetname, objectPath)
{
   ...
   switch(stringname)
   {
    ...
    if ((targetname.name == "A15F") || (targetname.name == "A9F") || (targetname.name == "A8F"))
    {
        ...
        libmakefile.writeLine("STARTERWARE_INCLUDE = $(PDK_INSTALL_PATH)/ti/starterware/include"); 

        var usbIncs = "-I. -I./src/include -I./src/usb_func/include -I$(STARTERWARE_INCLUDE) -I$(STARTERWARE_INCLUDE)/utils -I$(strip $(subst ;, -I,$(subst $(space),\\$(space),$(INCDIR)))) -I$("+tooldir+")/include ";

        print("createLibMake. device is: " + device.toString());

        if (targetname.name =="A15F") 
        {
            libmakefile.writeLine("INCS = -I./soc/" + device + " -I./src/dwc " + usbIncs);
        }
        else if (targetname.name =="A9F")
        {
            libmakefile.writeLine("INCS = -I./soc/" + device + " -I$(STARTERWARE_INCLUDE)/am43xx/" + device + " -I$(STARTERWARE_INCLUDE)/am43xx -I$(STARTERWARE_INCLUDE)/hw -I./src/dwc " + usbIncs);
        } 
        else if (targetname.name == "A8F")
        {
            libmakefile.writeLine("INCS = -I./soc/" + device + " -I$(STARTERWARE_INCLUDE)/am335x/" + device + " -I$(STARTERWARE_INCLUDE)/am335x -I$(STARTERWARE_INCLUDE)/hw -I./src/musb " + usbIncs);
        }
...
function makeAddObjects(srcString, makefilename, srcfiles, flags,fileExt, targetName, objDir)
{
 ...
 {
   makefilename.writeLine("\t$(AC) $(ASFLAGS_INTERNAL) $("+compileflagstring+") $(INTERNALDEFS) $(INCS) $< -o $@");
 }
