/******************************************************************************
 * FILE PURPOSE: USB LLD example files.
 ******************************************************************************
 * FILE NAME: module.xs
 *
 * DESCRIPTION: 
 *  This file contains the module specification for USB LLD example files.
 *
 * Copyright (C) 2009, Texas Instruments, Inc.
 *****************************************************************************/

/* Load the library utility. */
var libUtility = xdc.loadCapsule ("../build/buildlib.xs");

/**************************************************************************
 * FUNCTION NAME : modBuild
 **************************************************************************
 * DESCRIPTION   :
 *  The function is used to add all the source files in the example 
 *  directory into the package.
 **************************************************************************/
function modBuild() 
{
    /* Add all the .c files to the release package. */
    var exampleFiles = libUtility.listAllFiles (".c", "example", true);
    for (var k = 0 ; k < exampleFiles.length; k++)
        Pkg.otherFiles[Pkg.otherFiles.length++] = exampleFiles[k];

    /* Add all the .h files to the release package. */
    /* TBD: Adding all CCS project files by default  */
    var exampleFiles = libUtility.listAllFiles (".*", "example", true);
    for (var k = 0 ; k < exampleFiles.length; k++)    
        Pkg.otherFiles[Pkg.otherFiles.length++] = exampleFiles[k];
        
    var exampleFiles = libUtility.listAllFiles (".h", "example", true);
    for (var k = 0 ; k < exampleFiles.length; k++)
        Pkg.otherFiles[Pkg.otherFiles.length++] = exampleFiles[k];

    /* Add all the .cmd files to the release package. */
    var exampleFiles = libUtility.listAllFiles (".cmd", "example", true);
    for (var k = 0 ; k < exampleFiles.length; k++)
        Pkg.otherFiles[Pkg.otherFiles.length++] = exampleFiles[k];

    /* Add all the .lds (linker - bare metal) files to the release package. */
    var exampleFiles = libUtility.listAllFiles (".lds", "example", true);
    for (var k = 0 ; k < exampleFiles.length; k++)
        Pkg.otherFiles[Pkg.otherFiles.length++] = exampleFiles[k];

    /* Add all the .cfg files to the release package. */
    var exampleFiles = libUtility.listAllFiles (".cfg", "example", true);
    for (var k = 0 ; k < exampleFiles.length; k++)
        Pkg.otherFiles[Pkg.otherFiles.length++] = exampleFiles[k];

    /* Add all the make files to the release package. */
    var exampleFiles = libUtility.listAllFiles ("makefile", "example", true);
    for (var k = 0 ; k < exampleFiles.length; k++)
        Pkg.otherFiles[Pkg.otherFiles.length++] = exampleFiles[k];
    /* Add the .txt to the package */
    var exampleFiles = libUtility.listAllFiles (".txt", "example", true);
    for (var k = 0 ; k < exampleFiles.length; k++)
        Pkg.otherFiles[Pkg.otherFiles.length++] = exampleFiles[k];

    /* Add all the .mk files to the release package. */
    var mkFiles = libUtility.listAllFiles (".mk", "example", true);
    for (var k = 0 ; k < mkFiles.length; k++)
        Pkg.otherFiles[Pkg.otherFiles.length++] = mkFiles[k];

    /* Add all the .xs files to the release package. */
    var xsFiles = libUtility.listAllFiles (".xs", "example", true);
    for (var k = 0 ; k < xsFiles.length; k++)
        Pkg.otherFiles[Pkg.otherFiles.length++] = xsFiles[k];

    /* Add usb bulk py scripts to the release package. */
    var pyFiles = libUtility.listAllFiles (".py", "example", true);
    for (var k = 0 ; k < pyFiles.length; k++)
        Pkg.otherFiles[Pkg.otherFiles.length++] = pyFiles[k];

    /* udev rules */
    var ruleFiles = libUtility.listAllFiles (".rules", "example", true);
    for (var k = 0 ; k < ruleFiles.length; k++)
        Pkg.otherFiles[Pkg.otherFiles.length++] = ruleFiles[k];

    /* shell script files */
    var shellScriptFiles = libUtility.listAllFiles (".sh", "example", true);
    for (var k = 0 ; k < shellScriptFiles.length; k++)
        Pkg.otherFiles[Pkg.otherFiles.length++] = shellScriptFiles[k];

    /* ASM files */
    var asmFiles = libUtility.listAllFiles (".asm", "example", true);
    for (var k = 0 ; k < asmFiles.length; k++)
        Pkg.otherFiles[Pkg.otherFiles.length++] = asmFiles[k];
}
